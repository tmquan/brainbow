...
dependency — edge construction (numpy), the priority sort (`np.argsort`),
and the inherently-sequential union-find + mutex core (`_mws_core`) all
run on the **CPU**, the core JIT-compiled with `numba` over flat numpy
arrays.  The agglomeration is deliberately CPU-only: a CPU core is fast
at the serial pointer-chase, and keeping it off the device avoids
per-sample GPU syncs (which dominate under multi-GPU DDP) and extra
device-memory pressure (see §4.4).
...
agglomerator = MutexWatershed(offsets=None, n_pull=None,
                              strides=(1, 4, 4), size_filter=0)
ins = agglomerator(aff[B, n_offsets, D, H, W], fg_mask[B, D, H, W])  # -> [B, D, H, W] long
```

`MutexWatershed` processes one crop at a time and returns a
`[B, *spatial]` long label map (`0` = background), the drop-in contract
for the metric path.  Its `offsets` / `n_pull` default to
`brainbow.losses.AFFINITY_OFFSETS` / `N_PULL`.

### 4.4 Why CPU-only (and not on the GPU)

`MutexWatershed.forward` moves the whole batch to the host once
(`affinities.cpu().numpy()`) and then runs entirely in numpy / numba —
no per-sample device syncs.  Pushing the agglomeration onto the GPU is a
net loss in this pipeline:

- The union-find + mutex pass is **inherently serial** (online Kruskal
  with live mutex constraints), so a GPU can only run it in a single
  thread — far slower than one CPU core at branchy pointer-chasing.
- A "GPU build / sort + CPU core" split trades that away for a storm of
  small blocking GPU→CPU syncs (one per `.item()` / `.any()` / `.cpu()`),
  which, multiplied across DDP ranks, stalls every GPU on the host.
- The transient `torch.sort` / `torch.unique` / boolean-gather buffers
  add device-memory pressure exactly when the model is near its budget.

The single bulk transfer + sync-free CPU path is measurably faster here.
