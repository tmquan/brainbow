...
between voxels 3 and 4).  The attractive **single-step** (nearest-neighbour)
edges decide what merges; the repulsive **multi-step** (long-range) edges
...
Reading the panels: (1) the affinity graph — solid green attractive nn
edges, dashed red repulsive long-range edges; (2) all edges sorted by
priority; (3) the single rule; (4) the four strongest edges are attractive
within-object edges → two clusters form; (5) the repulsive `r(2,4)=0.88`
edge adds a **mutex** between the two clusters, which then makes the weak
attractive `a(3,4)=0.08` edge a no-op (blocked); (6) the parameter-free
