...
# Aliases: mws_np = exact sequential (numpy/numba); mws_cp = GPU (cupy).
...
class MutexWatershed(nn.Module):
    ...
    **non-differentiable** and used at eval / inference only.
    ...
    Dispatches per input (``backend``): a CUDA ``affinities`` runs the GPU
    cupy Boruvka :func:`mws_cp` zero-copy on the tensor (DLPack); a CPU
    tensor (or missing cupy) runs the exact numpy/numba :func:`mws_np`
    (one host transfer, then sync-free numba).  ``mws_cp`` is an
    approximation (ARI ~0.99 vs ``mws_np``); ``backend="cpu"`` forces the
    exact reference.
    ...
    Args:
        ...
        backend: ``auto`` (GPU on CUDA inputs, else CPU) / ``gpu`` / ``cpu``.
    ...
    def _use_gpu(self, affinities: torch.Tensor) -> bool:
        """Resolve the ``backend`` policy for this input."""
        if self.backend == "cpu":
            return False
        if self.backend in ("gpu", "cupy", "torch"):
            return affinities.is_cuda and _HAVE_CUPY
        # auto
        return affinities.is_cuda and _HAVE_CUPY
    ...
    def forward(
        ...
        # GPU path (mws_cp): cupy Boruvka, zero-copy from the CUDA tensor
        # via DLPack -- the dense affinity volume never leaves the device.
        if self._use_gpu(affinities):
...
__all__ = ["mutex_watershed", "mws_np", "mws_cp", "MutexWatershed"]
